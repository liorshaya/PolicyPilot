...
- [ ] `answer/v1` with context assembly; `POST /chat/sessions`; messages SSE (`token`, `citations`, `usage`, `done`) with resume; memory of the last 10 turns; marker parser and resolver; the four tools with validators, sandbox scope, 4 calls per turn and 1 simulate; the fixed not-covered sentence; chat screen with streaming, citation links, RTL (two-week version: the `decision` and `simulate` tools only; `stats` and `rules` are cut by rung 6)
