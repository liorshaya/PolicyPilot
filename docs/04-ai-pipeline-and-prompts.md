...
| Embedding | On publish, asynchronously, through `EmbeddingGateway`; `text-embedding-3-small` at 1536 dimensions on OpenAI, `bge-m3` at 1024 locally; the version's chunks are ready when its `embedding_status` is `READY`, and the chat refuses questions on a version still embedding with a clear message |
| Lexical index | PostgreSQL `tsvector` over the same chunk text with the `simple` dictionary (no stemming, which keeps Hebrew tokens intact) plus a trigram index for partial matches on rule ids and field names |
| Query | The user's question is embedded as-is (no rewriting in v1); a lexical query is built from its tokens, with rule ids, field names and numbers boosted |
| Fusion | Reciprocal rank fusion of the vector top 20 and the lexical top 20 with `k = 60`; the top 8 fused chunks are kept, always including the rule chunk for any rule id mentioned verbatim in the question |
| Threshold | If the best fused chunk's cosine similarity is below 0.35 and no rule id, decision id or field name from the question matched lexically, the API returns the fixed answer for the language ("the documents do not cover this question; try asking about a rule, a paragraph or a decision number") and logs the miss |
