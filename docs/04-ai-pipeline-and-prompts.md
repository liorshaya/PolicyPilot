...
**The output rule** (decided by the owner 2026-09-28, day 16): `{output}` says what the prompt returns. For a prompt with a schema it is `Respond with exactly one JSON object that conforms to the schema provided by the API, and nothing else.` followed on its own line by `(For the chat assistant: respond in plain text following the citation protocol.)`, the words the skeleton held until then, so every version already written renders byte for byte the prompt its cached and recorded calls were made from. For a prompt that streams text it is `Respond in plain text following the citation protocol.` alone: a version's system template takes `{conduct}`, the skeleton with the first rule, or `{textConduct}`, the skeleton with the second, and `answer/v3` is the first to take the second. The first rule sent the local model astray: on the `ollama` profile qwen3:14b read it as asking for JSON, wrapped its answers in a JSON object and made no tool call, and with the text rule alone it called `getDecision` on both runs of Q-01 (day 16).
...
**The threshold per embedding model** (decided 2026-09-28, day 16): cosines are not comparable across embedding models, so the `ollama` profile sets its own `policypilot.rag.min-score`, 0.58. On bge-m3 the labeled set's six refusal questions score 0.559 to 0.647 and its covered questions 0.471 to 0.794 (`docs/eval/retrieval-first-pass-ollama.md`), so no value separates them, as none does on OpenAI's embeddings, where 0.35 stops four of the six refusals and Q-14 with them. 0.58 is the lowest value that stops the demo's rate question, Q-04 at 0.572, and with it Q-22 and Q-30; it also stops Q-08 and Q-14 at 0.471, both answered by a tool, as any value that stops Q-04 would. The other refusals are the answer prompt's, as on OpenAI.
...
**`answer/v3`** (decided by the owner 2026-09-28, day 16): the system prompt takes the output rule of a prompt that streams text (Shared Conventions, The output rule), so its first rule says only `Respond in plain text following the citation protocol.`; the user prompt and every setting are `answer/v2`'s. Evaluation run 2 and the day 16 Ollama run found the local model answering in a JSON object and writing its tool calls as text instead of making them, because `answer/v2`'s first rule asks for JSON before its parenthesis exempts the chat; OpenAI's model follows the parenthesis. Its changelog compares it with `answer/v2` on both providers.
...
| --- | --- | --- |
| strong | `gpt-5.6-terra`, the balanced intelligence-and-cost model in the current API lineup; `gpt-5.6-sol` (the flagship) is the upgrade path if the author precision target is missed ([OpenAI models](https://developers.openai.com/api/docs/models)) | `qwen3:14b` with thinking disabled (`spring.ai.ollama.chat.think=false`) for the prompts that return JSON, so their output is the JSON object and nothing else; the answer thinks (Thinking on the local model, below) |
...
**Thinking on the local model** (decided by the owner 2026-09-28, day 16): the switch is off for the prompts that return JSON, so their output is the JSON object and nothing else, and on for the answer, which streams text. Without it qwen3:14b works a what-if out of the rules instead of calling `simulate`: asked Q-02 as `answer/v3` puts it, it answered from R-330 on each of three runs, and with thinking it called `getDecision` first on each of three, at about 300 thinking tokens and 16 s a move. Ollama returns the thinking apart from the answer, and Spring AI keeps it in the response's metadata, so it is never shown or cited.
