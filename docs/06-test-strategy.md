...
The matrix is a Markdown file in `docs/quality/traceability.md`, regenerated by a script from test annotations (`@Requirement("FR-8")` on JUnit tests, a `// @requirement FR-13` comment in Vitest and Playwright tests), so a requirement without a test shows up as an empty row in CI rather than in an interview. The script is `scripts/ci/traceability.py`: a JUnit class counts in the column of its level (`*Test` unit, `*IT` integration, `Recorded*IT` and `Eval*IT` recorded AI and evaluation), a Vitest file in the unit column and a Playwright file in the end-to-end column. Its list of rows still open only shrinks, and it is empty by day 16, when the Definition of Done asks for no empty row (decided 2026-09-27, day 15).
