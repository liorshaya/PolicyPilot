...
**Validation contexts**: the provenance rules depend on who produced the document. `AUTHORING`: every rule is the model's, so `analyst` and `pending` are both rejected. `CHANGE_PROPOSAL`: only the rules named in the patches are the model's; they may carry `quoted` or `pending`, never `analyst`; the other rules are untouched copies. `PUBLISH`: no `pending` may remain. `ANALYST_EDIT`: a person may create `analyst` provenance through the UI, which fills `actor` from the session. On approval of a change request, the system rewrites each `pending` into `analyst` with the approver as `actor`, `Change request <id>: <request text>` and, on the next line, the model's rationale as `note`, and the `changeRequestId` kept, so the audit trail records who approved what and why without ever trusting the model on either. The note is cut to the schema's 500 characters, the last of them an ellipsis, and the whole request stays in the change request and in the approval's audit entry.
...
**Version lineage**: a `ruleset_version` row is immutable; a new version records its `parent_version_id`, the change request that produced it names it as its result, and the approval's audit entry holds the structural diff. Version numbers are sequential per rule set. A version created by approving a change request is the only way to get from one published version to the next, and the base stays published, so its decisions and its chat go on working. A change proposed on a protected version is approved into the sandbox's own copy of the rule set: version 1 of the copy is the protected version as published, version 2 is the change, and the protected version never changes (decided 2026-09-24, day 13). A draft regenerated from a new policy version starts a new lineage only when the analyst chooses "replace", which the demo does not exercise.

**Structural diff**: computed by comparing the two documents by identity: `fields` by `name`, `rules` by `id`, `defaults` as a whole. For a modified rule the diff lists changed attributes with `from` and `to`, and for conditions it lists changed leaves by their JSON pointer, so the side-by-side view highlights `condition.value: 8000 → 9000` rather than the whole rule; a condition whose shape changed is one change at the pointer where the two shapes part, and a modified field lists its changed attributes the same way. Added and removed rules and fields are carried whole, and a modified one with both of its sides, so the view needs nothing else. The diff is stored in the audit entry as JSON and rendered in the UI from that JSON, never recomputed from the documents later. The first rule of the scripted request's diff, abridged (its `/actions` and `/provenance` change too, as whole attributes):
...
```json
{
  ...
  "rules": {
    ...
    "modified": [
      { "id": "R-170",
        "from": { "id": "R-170", "priority": 170, "condition": { "field": "monthly_income", "op": "lt", "value": 8000 } },
        "to": { "id": "R-170", "priority": 170, "condition": { "field": "monthly_income", "op": "lt", "value": 9000 } },
        "changes": [
          { "path": "/label", "from": "דחייה: הכנסה חודשית נטו נמוכה מ-8,000", "to": "דחייה: הכנסה חודשית נטו נמוכה מ-9,000" },
          { "path": "/condition/value", "from": 8000, "to": 9000 } ] } ] },
  "defaults": null
}
```

**Regression report**: after the patched copy validates, every decision the proposing sandbox made on the base version (the latest per case, and every decision not made from a stored case) is re-evaluated against the copy; the report counts the decisions it re-evaluated and lists `{ decisionId, caseNo, before, after, decidingRuleBefore, decidingRuleAfter }` for every flipped outcome, with `caseNo` null for a decision not made from a stored case and `after` `error` when the copy cannot decide the input, plus counts per transition (`approve → reject: 12`). A sandbox that has decided nothing on the base version gets an empty report that says so (decided 2026-09-24, day 13: the stored decisions, not a case set, because a version's decisions are what a change would change). The demo fixture set is generated so that, for the request above, exactly 12 of the 200 cases flip from `approve` or `refer` to `reject`, which is the number the presenter reads out.
