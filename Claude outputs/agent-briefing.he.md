...
| ה-fixtures | `fixtures/` | ה-JSON Schema של ה-DSL, מדיניות ההלוואות עם סט החוקים המפורסם, 200 המקרים והתוצאות הצפויות, בקשת השינוי המתוסרטת, סוויט התאימות C-01 עד C-31, 32 קבצי `invalid-*.json`, סט ההערכה (18 מדיניות, 30 שאלות, 6 בקשות שינוי), מימוש הרפרנס בפייתון והגנרטור |
...
9. **מרג'ים רק ירוק.** PR נכנס ל-`main` עם שלבי CI 1 עד 6 ירוקים (7 כשנגעו ב-`web/` או בחוזה של ה-API); `main` ירוק נפרס לבד ל-Railway ול-Vercel. pipeline אדום מתוקן לפני שכותבים בדיקה חדשה.
...
| --- | --- | --- | --- |
| 1 | ריפו ושלדים | מבנה המונו-ריפו (`api/`, `web/`, `fixtures/`, `docs/`, `.github/workflows/`); `api` על Java 21, Spring Boot 4.0.x ו-Spring AI 2.0.x BOM עם Flyway, Actuator ו-springdoc; אחת עשרה החבילות ריקות תחת `com.liorshaya.policypilot`; JaCoCo ו-PIT עם הספים של מסמך 6; `web` על React 19, TypeScript strict, Vite, TanStack Query, Vitest, Playwright, MSW, ESLint; `make up` כפקודה האחת אחרי `docker compose up` | ArchUnit: כל חץ בטבלת המודולים, שום דבר לא תלוי ב-`web`, Spring AI רק ב-`ai.adapter`, איסור על `ProcessBuilder`, `Runtime.exec`, `javax.script` ו-`@JsonTypeInfo`; context-loads לכל פרופיל (`openai`, `ollama`, עם `cloud`); Vitest smoke; Playwright שדף השער נטען |
| 2 | Docker Compose | PostgreSQL 16 עם pgvector על volume, Ollama עם סקריפט משיכה ל-`qwen3:14b` ו-`bge-m3`, אימג' ה-API מ-`api/Dockerfile`, שרת הפיתוח של web; Flyway V1 עם `CREATE EXTENSION IF NOT EXISTS vector` | מחלקת Testcontainers בסיסית לבדיקות `*IT` על אימג' pgvector; בדיקת אינטגרציה ש-Flyway רץ וההרחבה קיימת; הרצת compose מתוזמנת מול סף 5 הדקות |
...
2. Repository layout (api/, web/, fixtures/, docs/, docker-compose.yml, .github/workflows/), one line each, from docs/02-architecture.md "Deployment Topology".
