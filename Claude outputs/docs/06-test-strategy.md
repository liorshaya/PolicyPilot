...
```
api/src/test/java/com/liorshaya/policypilot/
...
**Versioning**: fixtures carry the DSL version in their file (`dslVersion`), and a schema change bumps it; the `schemas/` copies are compared with the API resources in CI so the fixtures and the running code cannot drift apart; recordings are keyed by prompt version, so old recordings stay valid for old prompts and are pruned when a prompt version is retired.
...
**Required checks for merging to `main`**: stages 1 to 6 on every pull request, stage 7 on pull requests that touch `web/` or the API contract; `main` is deployed by Railway and Vercel only from a green build, and the image deployed is the digest stage 6 produced.
