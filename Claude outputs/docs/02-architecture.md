...
**Error envelope**: `{ "code": "RULESET_INVALID", "message": "...", "details": [ { "path": "rules[3].condition", "problem": "unknown field 'monthly_incom'" } ], "traceId": "..." }`; codes are an enum shared with the client, and validation failures use HTTP 422, provider failures 503 with `code: PROVIDER_UNAVAILABLE`, missing or wrong access code 401, rate limit 429 with `Retry-After`.
...
| Folder | Contents |
| --- | --- |
| `web/src/api/` | Generated TypeScript client from the OpenAPI document (openapi-typescript), the SSE helper, the error envelope type, the access-code exchange, and the X-PolicyPilot-Client header on every request |
| `web/src/features/policy/` | Policy editor (textarea with paragraph numbering), upload, version list |
| `web/src/features/rules/` | Decision table (TanStack Table), rule detail drawer with provenance, JSON view, findings panel, publish dialog |
| `web/src/features/decide/` | Case form, batch runner, outcome dashboard, trace view |
| `web/src/features/chat/` | Chat panel with streamed tokens, citation chips that open the rule or paragraph, tool-call badges |
| `web/src/features/change/` | Change request box, diff view (side by side, rule level), regression report, approve and reject |
| `web/src/features/audit/` | Audit log, version timeline |
| `web/src/shared/` | Layout, provider badge, access-code gate, i18n, RTL utilities, design tokens |
...
| Path | Contents |
| --- | --- |
| `api/` | Maven project, `Dockerfile` (multi-stage: build with Maven and Temurin 21, run on a JRE image), `src/main/resources/prompts/`, Flyway migrations |
| `web/` | Vite project, `vercel.json` with the SPA rewrite |
...
| `docs/` | Documents 1 to 7 and the progress checklist exported as Markdown with a README index, worklog.md, architecture diagrams, docs/quality/ (coverage, mutation and traceability reports) and docs/eval/ (evaluation reports) |
| `docker-compose.yml` | `db` (`pgvector/pgvector:pg16`), `ollama` (optional profile), `api` (built from `api/`), `web` (Vite dev server or nginx) |
...
| --- | --- | --- |
| Build | Dockerfile in `api/`, root directory set to `api/` | Root directory `web/`, `npm run build`, output `dist/` |
