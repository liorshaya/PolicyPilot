...
Run: python3 check.py   (expects ruleset-1.0.schema.json, consumer-lending.v1.json, policy-he.txt)
"""
...
# ----------------------------------------------------------------------------- schema
schema = json.load(open("ruleset-1.0.schema.json"))
...
if __name__ == "__main__":
    rs = json.load(open("consumer-lending.v1.json", encoding="utf-8"))
    paras = [p.strip() for p in open("policy-he.txt", encoding="utf-8").read().split("\n\n") if p.strip()]
    ...
    json.dump(d, open("sample-decision.json", "w"), ensure_ascii=False, indent=2)
