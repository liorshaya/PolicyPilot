...
def structural_layer(rs):
    ...
    for r in rules:
        for a in r["actions"]:
            if a["type"] == "set" and a["field"] in fields:
                ...
                deps.setdefault(a["field"], set()).update(x for x in acc if fields.get(x, {}).get("derived"))
    def has_cycle(start):
    ...
    for r in rules:
        ...
        used |= acc
        own_sets = {a["field"] for a in r["actions"] if a["type"] == "set"}
        for name in sorted(acc):
            if fields.get(name, {}).get("derived") and name not in own_sets and name in setters and setters[name] >= r["priority"]:
                out.append(F("DERIVED_ORDER", "/rules", f"{r['id']} reads {name}, set at priority {setters[name]}"))
        divs = set()
        for a in r["actions"]:
