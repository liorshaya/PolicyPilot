...
fixtures/
  ...
  eval/recordings/<provider>/<prompt>/<version>/   what a live run answered, keyed by the hash of the
                               rendered prompts, with those prompts beside it; replayed offline by
                               RecordedGateway and never regenerated by CI (Document 6, Recordings)
